...
public:
    ...
    Medico(int ID, int numColegiacion, int numIdentidad, int numTelefono,
           string nombre, string especialidad, string fechaNacimiento, string eMail, bool disponible) :
        ID(ID), numColegiacion(numColegiacion), numIdentidad(numIdentidad), numTelefono(numTelefono),
        nombre(nombre), especialidad(especialidad), fechaNacimiento(fechaNacimiento), eMail(eMail), disponible(disponible) {}
